...
function initiateSearch() {
  ...
  if (inputTags.length == 1 && inputTags[0] == "") {
    // If no tags have been entered
    $("#cards_cont").show(); // Show all the resources
    $(".re_card").show();
  } else if (inputTags.length == 1 && inputTags[0] == "*") {
    // '*' wildcard
    resources.each(function () {
      ...
      tags.each(function (i) {
        ...
        if (curTag != null || curTag.length > 0) resTagArray.push(curTag);
      });

      if (!checkFilters(resTagArray, filters)) $(this).hide();
    });
  } else {
    resources.each(function () {
      let tags = $(this).find(".re_tags");
      ...
      if (
        containsOtherArray(inputTags, resTagArray) ||
        curResTitle.includes(inputTags[0])
      ) {
        if (useFilters && !checkFilters(resTagArray, filters)) $(this).hide();
        else {
          $(this).show();
          $("#cards_cont").show();
        }
    ...
    if ($("#cards_cont").children(":visible").length == 0)
      $("#cards_cont").hide();
    else $("#cards_cont").show();
  }
}

function containsOtherArray(thisArray, otherArray) {
  for (let i = 0; i < thisArray.length; i++) {
    if (otherArray.indexOf(thisArray[i]) === -1) return false;
  }
  return true;
}
...
function checkFilters(tags, filter) {
  ...
  tags.forEach(function (i) {
    if (filter.includes(i)) include = false;
  });
...
function initTagSearch(event, el) {
  if (event.shiftKey) {
    // Shift + Click initiates search with all the tags up to the one the user selected
  ...
  } else $("#search_input").val(el.innerHTML);
